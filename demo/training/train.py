...
def preprocess_images(data_yaml: str, apply_clahe: bool = False):
    ...
    for img_path in tqdm(list(train_images.glob('*.jpg')) + list(train_images.glob('*.png'))):
...
사용 예시:
  ...
  python train.py --tune --model s --iterations 20
    ...
    parser.add_argument('--device', type=str,
                        default='0',
                        help='GPU 장치 (0) 또는 cpu')
    parser.add_argument('--name', type=str,
                        default='egg_classifier_advanced',
                        help='실험 이름')
    ...
    parser.add_argument('--optimizer', type=str,
                        ...
                        help='옵티마이저 선택')
    parser.add_argument('--no-advanced-aug',
                        action='store_true',
                        help='고급 증강 비활성화')
    ...
    # 전처리 옵션
    parser.add_argument('--preprocess-clahe',
                        action='store_true',
                        help='CLAHE 전처리 적용 (선택)')
